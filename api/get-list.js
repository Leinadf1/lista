...
function buildM3U(channel) {
    let out = '';
    out += `#EXTINF:-1 tvg-logo="${channel.logo}" group-title="${channel.group_title || 'EUROSPORT'}"`;
    if (channel.tvg_id) out += ` tvg-id="${channel.tvg_id}"`;
    out += `,${channel.name}\n`;
