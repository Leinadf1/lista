    ...
    try {
        ...
        try {
            ...
            if (!daznGistId) {
                ...
                // Se il tuo GIST_TOKEN non ha scope 'gist', puoi usare un token separato:
                // const token = process.env.DAZN_GIST_TOKEN || process.env.GIST_TOKEN;
                const token = process.env.GIST_TOKEN; // Assicurati che abbia lo scope 'gist'
                const apiUrl = `https://api.github.com/gists/${daznGistId}`;
                ...
                if (daznResponse.ok) {
                    ...
                    if (daznFile && daznFile.content) {
                        ...
                        daznContent = daznFile.content.replace(/^#EXTM3U\s*\n?/i, '').trim();
                        console.log("[DAZN] Contenuto ricevuto, lunghezza:", daznContent.length);
                    } else {
        ...
        // Aggiunge DAZN (se presente) – senza più usare DAZN_FISSI
        if (daznContent) {
